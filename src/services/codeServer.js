...
const sendCode = async ( phone, provider= 'Vonage' )=>{
    try {
        ...
        const doc = await VerificationCode.create({
            phone,
            code: hash,
            provider,
        ...
        logger.info(`Codigo Generado id: ${doc._id} para ${phone}, expira en ${expiracion}`);

        const mensaje =`Tu codigo de verificacion es: ${codigo}`;
        const enviado= await sendSMS(phone,mensaje);

        if(!enviado){
            logger.warn(`SMS no enviado a ${phone}. sigue en estado:Pendiente`);
        }
        return{
            ok:true,
            mensaje: 'Codigo generado y registrado',
            registroId:doc._id
        };
